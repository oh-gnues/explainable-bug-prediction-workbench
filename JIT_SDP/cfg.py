...
class CFG:
    ...
    def compute_gen_kill(self, node: CFGNode):
        ...
        def find_variables(expression: Node, where="use"):
            if isinstance(expression, MemberReference):
                ...
                if ({"++", "--"} & (set(expression.prefix_operators or []) | set(expression.postfix_operators or []))):
    ...
    def _control_flow(self, parent: CFGNode, statement: Node) -> CFGNode:
        ...
        if isinstance(statement, (list, tuple)):
            ...
            # No statement
            # print(type(statement), parent.__dict__)
    ...
    def _process_for(self, parent: CFGNode, for_statement: ForStatement) -> CFGNode:
        ...
        if isinstance(for_statement.control, javalang.tree.ForControl):
        ...
        elif isinstance(for_statement.control, javalang.tree.EnhancedForControl):
    ...
    def _resolve_breaks(self):
        ...
        for label, break_nodes in self.unresolved_breaks.items():
            label_node = self.labels.get(label, None)
            if not label_node:
                print(self.method.line_numbered_snippet)
    ...
    def _process_switch(
        ...
        last_switch_node = self._control_flow(switch_expression, switch_statement.expression)
...
def create_use_def_graph(cfg):
    use_def_graph = UseDefGraph()
    for node in cfg.nodes.values():
        if node.in_set & node.uses:
            for variable in node.in_set & node.uses:
                use_def_graph.add_edge(UseDefNode(variable.assigner), UseDefNode(node))

    return use_def_graph
